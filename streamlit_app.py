...
st.set_page_config(page_title="Fokker‑Planck Equation – Entropy Production", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Fokker‑Planck Equation – Entropy Production</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Probability density evolution | Macro‑modulated density | Entropy production rate as signal</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Fokker‑Planck")
...
st.sidebar.markdown(f"**Grid points:** {config.GRID_POINTS} | **All macros used**")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'fokker_planck_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Entropy production: {etf['fp_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Entropy Production"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Entropy Production", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Entropy Production Rate (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Fokker‑Planck equation** governs the evolution of probability density over time.
        - We split the window into two halves and estimate the density of returns in each.
        - The **entropy production rate** is the KL divergence between the two densities divided by time.
        - Macro variables modulate the density evolution via a composite factor.
        - High entropy production → regime change / instability → potential alpha.
        - Low entropy production → stable distribution → predictable.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the entropy production scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Fokker‑Planck Equation | Entropy production from density evolution")
